...
Evaluates the RAG generation pipeline using either:
  ...
  - gemini:  Qdrant retrieval → context → Gemini directly
...
Usage:
    ...
    python -m evaluation.generation_eval --generator gemini --gemini-model gemini-3.5-flash-lite
    python -m evaluation.generation_eval --generator gemini --gemini-model gemini-3.5-flash-lite --limit 30
...
_ROOT     = _PROJECT.parent                           # → project root (contains backend/)
# Add backend/ so `evaluation.*` imports work
...
# Default Gemini model — override via --gemini-model on the CLI
...
def _build_gemini_client(api_key: str):
    ...
    Called ONCE before the evaluation loop — reused for all questions.
    """
    try:
...
def _gemini_generate(
    ...
    Args:
        ...
        client:         pre-built genai.Client (created once, reused per question)
        model:          Gemini model identifier (e.g. 'gemini-3.5-flash-lite')

    Returns:
        Generated answer string (stripped)

    Raises:
        RuntimeError on API failure — never falls back to Ollama
    """
...
def run_generation_eval(
    ...
    If one question fails (retrieval error, API error, etc.) it is recorded
    with an 'error' field and the loop continues — the run is never aborted.
    """
    from backend.rag.retriever import retrieve

    # Build Gemini client ONCE — avoids per-question SSL context overhead
    gemini_client = None
    if generator == "gemini":
        gemini_key    = _check_gemini_key()   # raises if missing
        gemini_client = _build_gemini_client(gemini_key)
    ...
    for i, q in enumerate(questions, 1):
        ...
        try:
            ...
            # Record the failure but continue to next question
            rows.append({
                "id":          q["id"],
                "paper_label": label,
        ...
        try:
            if generator == "gemini":
                answer = _gemini_generate(
                    chunk_texts, q["question"], gemini_client, gemini_model
                )
            else:
        ...
        row = {
            ...
            "generator":       generator,
            "gemini_model":    gemini_model if generator == "gemini" else None,
            "question":        q["question"],
            "answer":          answer[:600],
            "reference":       ref[:300],
            "token_f1":        round(f1, 4),
...
def write_latex(agg: dict, path: Path, generator: str, gemini_model: str = "") -> None:
    f1  = agg.get("token_f1",           "?")
    ...
    if generator == "gemini" and gemini_model:
        gen_label = f"Gemini {gemini_model} (RAG)"
    elif generator == "gemini":
        gen_label = "Gemini (RAG)"
    else:
        gen_label = "AstroNexus (Orch.)"
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        dest="gemini_model",
        help=f"Gemini model ID to use (default: {_DEFAULT_GEMINI_MODEL})",
    )
    ...
    # Include model name slug in filename for Gemini runs
    model_slug = gemini_model.replace("/", "-") if generator == "gemini" else ""
    stem = f"results_{generator}{'_' + model_slug if model_slug else ''}_{ts}"
    ...
    write_latex(agg, latex_path, generator, gemini_model)
