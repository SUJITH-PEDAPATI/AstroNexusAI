"""
Full Pipeline Test — Week 2 (RAG) + Week 3 (Knowledge Graph).
...
def run(file_path: str) -> None:
    ...
    print(f"\n{'='*60}")
    print(f"[Week 3] Building Knowledge Graph")
    print(f"{'='*60}")
    ...
    print(f"  MATCH (n) RETURN n   ← to visualize the full graph")

    # ── Save all outputs ───────────────────────────────────────────────────────
    out_dir1 = Path("output/test_folder/ingestion_output")
    out_dir1.mkdir(exist_ok = True)

    out_dir2 = Path("output/test_folder/embedding_output")
    out_dir2.mkdir(exist_ok = True)

    out_dir3 = Path("output/test_folder/chunking_output")
    ...
    with open(out_dir3 / "chunks_result.json", "w", encoding="utf-8") as f:
        json.dump([c.model_dump() for c in chunks], f, indent=2, default=str)

    with open(out_dir2 / "embedded_result.json", "w", encoding="utf-8") as f:
        json.dump(
            [
                {
                    **ec.chunk.model_dump(),
    ...
    out_dir5 = Path("output/db_results")
    out_dir5.mkdir(exist_ok = True)
    with open(out_dir5 / "graph_result.json", "w", encoding="utf-8") as f:
    ...
    print(f"   Week 3 → output/graph_result.json")
    print(f"\n  Week 2 (RAG) + Week 3 (Knowledge Graph) complete.\n")
